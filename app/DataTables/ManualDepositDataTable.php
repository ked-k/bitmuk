...
class ManualDepositDataTable extends DataTable
{
    /**
    ...
    public function dataTable($query)
    {
        ...
        return $dataTable->addColumn('full_name', 'backend.transaction.full_name')
            ...
            ->editColumn('status', 'backend.deposit.status')
            ->rawColumns(['status', 'prof']);
    }
    ...
    {
        return $model->where('type', 'manual_deposit')->latest()->newQuery();
    }
...
//            ->addAction(['width' => '120px', 'printable' => false, 'title' => 'action' ])
    ...
    {
        return 'manual_deposit_datatable_' . time();
    }
}
